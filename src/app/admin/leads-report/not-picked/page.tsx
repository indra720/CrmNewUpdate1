...
import { updateAdminLeadStatus, addAdminLead } from '@/lib/api';
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/accounts/api/not-picked/`, {
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch not picked leads');
        }
