...
import { addAdminLead, updateAdminLeadStatus } from '@/lib/api';
...
function OtherLocationLeadsPage() {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      ...
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/accounts/api/maybe/`, {
          headers: {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch other location leads');
        }
  ...
  const handleAddLeadSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Re-fetch leads after adding a new one
      const fetchLeads = async () => {
        setLoading(true);
        try {
          const token = localStorage.getItem('authToken');
              ...
              <Table>
                ...
                <TableBody>
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={table.getAllColumns().length} className="h-24 text-center">
                        No leads found for other locations.
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
