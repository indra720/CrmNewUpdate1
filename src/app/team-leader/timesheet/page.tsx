...
const TimeSheetPage = () => {
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/accounts/api/activityteamlogs/?page=${page}`,
      {
        headers: {
    ...
    if (response.ok) {
      ...
      setLogs(data.logs || []);                // FIXED
      setTotalCount(data.total_items || 0);    // FIXED
      setTotalPages(data.total_pages || 1);    // FIXED
    }
  } catch (error) {
